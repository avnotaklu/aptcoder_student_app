    ...
    final user = await creds.fold<Future<Either<Failure, AuthUser>>>((Failure l) => Future.value(Left(l)),
      ...
      if (user.additionalUserInfo != null) {
        ...
        if (user.additionalUserInfo!.isNewUser == false) {
          ...
          return type.fold<Either<Failure, AuthUser>>(
              (l) => Left(l),
              (type) =>
                  // NOTE: displayName and email will never be null in google sign in
                  Right(AuthUser(user.user!.displayName!, user.user!.email!, user.additionalUserInfo!.isNewUser, type,
